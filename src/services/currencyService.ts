...
export const currencyService = {
  ...
  async getAllCurrencies(): Promise<Currency[]> {
    try {
      const response = await fetch(`${BASE_URL}/get_all_currencies?api_key=${API_KEY}`);
   ...
   */
  async getExchangePair(fromCurrency: string, toCurrency: string, amount?: string): Promise<CurrencyPair> {
    try {
   ...
   */
  async getRange(fromCurrency: string, toCurrency: string): Promise<{ min: string; max: string }> {
    try {
      const params = new URLSearchParams({
        api_key: API_KEY,
        currency_from: fromCurrency,
   ...
   */
  async getEstimatedExchangeTime(fromCurrency: string, toCurrency: string): Promise<number> {
    try {
      const params = new URLSearchParams({
        api_key: API_KEY,
        currency_from: fromCurrency,
      ...
      const response = await fetch(`${BASE_URL}/get_estimated_exchange_time?${params}`);
  ...
  async createExchange(params: {
    ...
    try {
      const requestParams = new URLSearchParams({
        api_key: API_KEY,
        ...params,
        fixed: (params.fixed ?? false).toString(),
      });

      const response = await fetch(`${BASE_URL}/create_exchange?${requestParams}`);
